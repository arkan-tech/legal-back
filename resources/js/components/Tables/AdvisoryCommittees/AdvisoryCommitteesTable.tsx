...
const AdvisoryCommitteesTable = ({
    ...
    useEffect(() => {
        setFilteredData(
            data.filter(
                (advisoryCommittee) =>
                    advisoryCommittee.name
                        .toLowerCase()
                        .includes(searchQuery.toLowerCase()) ||
                    advisoryCommittee.id == searchQuery
            )
        );
        setCurrentData(
            data
        ...
        >
            <div className="grid grid-cols-12 gap-2">
                ...
                <div className="col-span-6 flex items-center justify-end">
                    <Link
                        href="/newAdmin/advisory-committees/create"
            ...
            <div className="max-w-full overflow-x-auto">
                <table className="w-full table-auto">
                    ...
                    <tbody>
                        {currentData.map((advisoryCommittee, key) => (
                            <tr key={key}>
                                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                    <h5 className="font-medium text-black dark:text-white">
                                        {advisoryCommittee.id}
                                    </h5>
                                </td>
                                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                    <h5 className="font-medium text-black dark:text-white">
                                ...
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <div className="flex items-center gap-5">
                                        <Link
                                            href={`/newAdmin/advisory-committees/${advisoryCommittee.id}`}
                                        ...
                                        <button
                                            onClick={() =>
                                                router.get(
                                                    `/admin/advisory-committees/delete/${advisoryCommittee.id}`,
                                                    {},
                                                    {
                                                        onSuccess: () => {
                                                            toast.success(
...
export default AdvisoryCommitteesTable;
