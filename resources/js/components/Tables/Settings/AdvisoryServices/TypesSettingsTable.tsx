...
const TypesSettingsTable = ({
    ...
    const handleToggle = async (id) => {
        ...
        try {
            const response = await axios.post(
                `/newAdmin/settings/advisory-services/types/${id}/toggle`,
                {
    ...
    useEffect(() => {
        console.log(selectedAdvisoryService);
        setFilteredData(
            data.filter(
                (type) =>
                    type.name
    ...
    }, [currentPage, searchQuery, selectedAdvisoryService]);
    return (
        <div
            style={{ direction: "rtl" }}
            ...
            <div className="grid grid-cols-12 gap-2">
                ...
                <div className="col-span-4">
                    ...
                    <select
                        ...
                        value={selectedAdvisoryService}
                        onChange={(e) =>
                            setSelectedAdvisoryService(e.target.value)
                        }
                    >
                        <option value="">الكل</option>
                        {advisoryServices.map((advisoryService) => (
                            <option value={advisoryService.id}>
                                {advisoryService.title}
                            </option>
                        ))}
                    </select>
                </div>
                ...
                <div className="col-span-2 flex items-center justify-end">
                    <Link
                        href="/newAdmin/settings/advisory-services/types/create"
                            ...
                            <tr key={key}>
                                ...
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <p className="text-blue-700 dark:text-white">
                                        الأدنى: {type.minPrice}
                                    </p>
                                    <p className="text-blue-700 dark:text-white">
                                        الأعلى: {type.maxPrice}
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                    <h5 className="font-medium text-black dark:text-white">
                                ...
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <div className="flex items-center gap-5">
                                        <Switcher
                                            enabled={enabledStates[type.id]}
                                            handleToggle={handleToggle}
                                            id={type.id}
                                        />
                                        <Link
                                            href={`/newAdmin/settings/advisory-services/types/${type.id}`}
...
export default TypesSettingsTable;
