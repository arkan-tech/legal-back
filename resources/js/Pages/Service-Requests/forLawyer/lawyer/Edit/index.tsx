    ...
    const saveData = async (formData) => {
        try {
            const response = await axios.post(
                "/admin/lawyers/service-request/final-replay/forLawyer",
                formData,
                {
                    headers: {
            ...
            if (response.status == 200) {
                ...
                if (formData.get("for_admin") != 2) {
                    router.get(
                        `/newAdmin/services-requests/lawyer/${formData.get(
                            "id"
                        )}`
                    );
                } else {
    ...
    return (
        <DefaultLayout>
            ...
            <ServiceRequestForm
                ...
                returnRoute={`/newAdmin/services-requests/for-lawyer/lawyer`}
            />
        </DefaultLayout>
    );
}
