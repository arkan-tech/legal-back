    ...
    const saveData = async (formData) => {
        try {
            const response = await axios.post(
                "/newAdmin/advisory-services-requests/clients/final-replay",
                formData,
                {
                    headers: {
            ...
            if (response.status == 200) {
                ...
                if (formData.get("for_admin") == 2) {
                    router.visit(
                        `/newAdmin/advisory-services-requests/for-lawyer/client/${formData.get(
                            "id"
                        )}`
                    );
                } else {
    ...
    return (
        <DefaultLayout>
            ...
            <AdvisoryServiceRequestForm
                ...
                returnRoute={`/newAdmin/advisory-services-requests/client`}
            />
        </DefaultLayout>
    );
}
