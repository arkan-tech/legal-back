    ...
    const saveData = async (formData) => {
        try {
            const response = await axios.post(
                "/newAdmin/advisory-services-requests/lawyers/final-replay",
                formData,
                {
                    headers: {
            ...
            if (response.status == 200) {
                ...
                if (formData.get("for_admin") == 2) {
                    router.get(
                        `/newAdmin/advisory-services-requests/for-lawyer/lawyer/${formData.get(
                            "id"
                        )}`
                    );
                } else {
    ...
    return (
        <DefaultLayout>
            ...
            <AdvisoryServiceRequestForm
                ...
                returnRoute={`/newAdmin/advisory-services-requests/lawyer`}
            />
        </DefaultLayout>
    );
}
