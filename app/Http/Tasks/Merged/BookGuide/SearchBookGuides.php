...
class SearchBookGuides extends BaseTask
{
    ...
    public function run(Request $request)
    {
        ...
        $lawGuideMainCategoryId = $request->bookGuideCategoryId;
        $lawGuideSubCategoryId = $request->bookGuideSubCategoryId;
        $lawGuideMainCategories = [];
        $lawGuide = [];
        $laws = [];
        $mainCategoriesPage = $request->query('bookCategoriesPage') ?? 1;
        $lawGuidePage = $request->query('bookGuidePage') ?? 1;
        $lawsPage = $request->query('sectionsPage') ?? 1;
        $relatedLawGuidesPage = $request->query('relatedBookGuidesPage') ?? 1;
        $perPage = $request->query('perPage') ?? 10;
        if (is_null($lawGuideMainCategoryId) and is_null($lawGuideSubCategoryId)) {
            $lawGuideMainCategories = BookGuideCategory::where('name_en', 'like', "%{$searchTerm}%")->orWhere('name_ar', 'like', "%($searchTerm)%")->paginate($perPage, page: $mainCategoriesPage);
            $lawGuide = BookGuide::where('name_en', 'like', "%{$searchTerm}%")->orWhere('name_ar', 'like', "%($searchTerm)%")->paginate($perPage, page: $lawGuidePage);
            $laws = BookGuideSection::where('name_en', 'like', "%{$searchTerm}%")->orWhere('name_ar', 'like', "%($searchTerm)%")->orWhere(
                'section_text_en',
                'like',
                "%{$searchTerm}%"
            )->orWhere(
                    ...
                    'like',
                    "%{$searchTerm}%"
                )->orWhere('changes_en', 'like', "%{$searchTerm}%")->orWhere('changes_ar', 'like', "%{$searchTerm}%")->paginate($perPage, page: $lawsPage);
            $relatedLawGuides = BookGuide::whereHas('sections', function ($query) use ($searchTerm) {
                $query->where('name_en', 'like', "%{$searchTerm}%")->orWhere('name_ar', 'like', "%{$searchTerm}%")->orWhere(
                    'section_text_ar',
                    'like',
                    "%{$searchTerm}%"
                )->orWhere(
                        ...
                        'like',
                        "%{$searchTerm}%"
                    )->orWhere('changes_en', 'like', "%{$searchTerm}%")->orWhere('changes_ar', 'like', "%{$searchTerm}%");
            })->paginate($perPage, page: $relatedLawGuidesPage);
        } else if (!is_null($lawGuideMainCategoryId) and is_null($lawGuideSubCategoryId)) {
            $lawGuide = BookGuide::where('name_en', 'like', "%{$searchTerm}%")->orWhere('name_ar', 'like', "%($searchTerm)%")->where('category_id', $lawGuideMainCategoryId)->paginate($perPage, page: $lawGuidePage);
            $laws = BookGuideSection::whereHas('bookGuide', function ($query) use ($lawGuideMainCategoryId) {
                $query->where('category_id', $lawGuideMainCategoryId);
            })->where('name_en', 'like', "%{$searchTerm}%")->orWhere('name_ar', 'like', "%($searchTerm)%")->orWhere(
                    'section_text_en',
                    'like',
                    "%{$searchTerm}%"
                )->orWhere(
            ...
            $relatedLawGuides = BookGuide::where('category_id', $lawGuideMainCategoryId)->whereHas('sections', function ($query) use ($searchTerm) {
                $query->where('name_en', 'like', "%{$searchTerm}%")->orWhere('name_ar', 'like', "%($searchTerm)%")->orWhere(
                    'section_text_en',
                    'like',
                    "%{$searchTerm}%"
            ...
            $laws = BookGuideSection::whereHas('bookGuide', function ($query) use ($lawGuideSubCategoryId) {
        ...
        $lawGuideMainCategoriesResource = $lawGuideMainCategories->map(function ($category) {
            return new BookGuideCategoryResource($category, true);
        });
        $bookGuideCategories = [
        ...
        $lawGuideResource = BookGuideResource::collection($lawGuide);
        $bookGuide = [
        ...
        $lawsResource = BookGuideSectionResource::collection($laws);
        $sections = [
            "data" => $lawsResource,
            "total" => $laws->total(),
        ...
        $relatedLawGuidesResource = $relatedLawGuides->map(function ($guide) {
            return new BookGuideResourceShort($guide, true);
        });
        $relatedBookGuide = [
        ...
        return $this->sendResponse(true, 'Sections Search', compact('bookGuideCategories', 'bookGuide', 'sections', 'relatedBookGuides'), 200);
    }
}
