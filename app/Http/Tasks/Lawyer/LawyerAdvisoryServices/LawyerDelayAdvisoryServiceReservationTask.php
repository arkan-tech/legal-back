...
class LawyerDelayAdvisoryServiceReservationTask extends BaseTask
{

    public function run(LawyerDelayAdvisoryServiceReservationRequest $request)
    {

        $lawyer = $this->authLawyer();

        $advisory_services_reservation = LawyerAdvisoryServicesReservations::with('service')->where('reserved_lawyer_id', $lawyer->id)->where('id', $request->client_advisory_services_reservation_id)->first();
        ...
        $LawyerAdvisoryServicesAppointment = LawyerAdvisoryServicesAppointment::where('lawyer_id', $lawyer->id)->pluck('lawyer_advisory_services_reservation_id')->toArray();
        if (in_array($advisory_services_reservation->id, $LawyerAdvisoryServicesAppointment)) {
        ...
        $reservation = LawyerAdvisoryServicesAppointment::create([
            'lawyer_id' => $lawyer->id,
            'lawyer_advisory_services_reservation_id' => $advisory_services_reservation->id,
        ...
        $params = array(
            ...
            'bill_city' => 'City',
            'bill_addr1' => 'City',
            'return_auth' => $Domain . '/api/lawyer/advisory-services/complete/payment/' . $advisory_services_reservation->id,
            'return_can' => $Domain.'/api/lawyer/advisory-services/cancel/payment/' . $advisory_services_reservation->id ,
            'return_decl' => $Domain.'/api/lawyer/advisory-services/declined/payment/' . $advisory_services_reservation->id ,
