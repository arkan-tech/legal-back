...
class LawyerGetMyReservationsTaskLawyer extends BaseTask
{

    public function run(Request $request)
    {
        $lawyer = $this->authLawyer();
        $reservations = Reservation::where('reserved_from_lawyer_id', $lawyer->id)->where('transaction_complete', 1)->whereNotNull('lawyer_id')->get();
