...
namespace App\Http\Tasks\Client\AdvisoryServices\Lawyer;
...
class ClientCreateLawyerReservationTask extends BaseTask
{

    public function run(ClientCreateReservationLawyerRequest $request)
    {
        $client = $this->authClient();
        $lawyer = Lawyer::where('id', $request->lawyer_id)->firstOrFail();
        ...
        if ($advisory->advisoryService->need_appointment == 1) {
            ...
            if ($payment_method->payment_method == 2) {
                ...
                $params = array(
                    ...
                    'bill_city' => 'City',
                    'bill_addr1' => 'City',
                    'return_auth' => $Domain . '/api/payments/callback/client/advisory-services/' . $reservation->id,
                    'return_can' => $Domain . '/api/payments/callback/client/advisory-services/' . $reservation->id,
                    'return_decl' => $Domain . '/api/payments/callback/client/advisory-services/' . $reservation->id,
            ...
            if ($payment_method->payment_method == 2) {
                ...
                $reservations_count = ClientAdvisoryServicesReservations::where('client_id', $client->id)->where('advisory_services_id', $request->advisory_services_id)->count();
        ...
        $reservation = ClientAdvisoryServicesReservations::find($reservation->id);
