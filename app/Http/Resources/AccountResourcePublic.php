...
use App\Http\Resources\API\Lawyer\GeneralData\Section\LawyerSectionResourcePrivate;
...
class AccountResourcePublic extends JsonResource
{
    /**
    ...
    {
        $nextLevel = null;
        if ($this->clientGamification() && $this->clientGamification()->level) {
        ...
        if ($isLawyer) {
            ...
            $lawyerData = [
                'name' => $name,
                'about' => optional($this->lawyerDetails)->about ?? 'لا يوجد تفاصيل',
                'accurate_specialty' => optional($this->lawyerDetails)->accurate_specialty ? new LawyerAccurateSpecialtyResource($this->lawyerDetails->AccurateSpecialty) : null,
                'general_specialty' => optional($this->lawyerDetails)->general_specialty ? new LawyerGeneralSpecialtyResource($this->lawyerDetails->GeneralSpecialty) : null,
                'degree' => optional($this->lawyerDetails)->degree ? new LawyerDegreesResource($this->lawyerDetails->degreeRel) : null,
                'is_favorite' => in_array($this->id, $fav_lawyers) ? 1 : 0,
                'special' => optional($this->lawyerDetails)->is_special,
                'logo' => optional($this->lawyerDetails)->logo,
                'sections' => LawyerSectionResourcePrivate::collection(optional($this->lawyerDetails)->SectionsRel ?? []),
                'permissions' => $subscription && $subscription->package ? PermissionResource::collection(optional($subscription->package)->permissions ?? []) : [],
                'hasBadge' => $subscription && $subscription->package && $subscription->package->permissions->contains('id', 1)
                    ? 'blue'
                    : (
                        $subscription && $subscription->package && $subscription->package->permissions->contains('id', 7)
                        ? 'gold'
                        : null
                    ),
