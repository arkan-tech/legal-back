...
class PackageResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'number_of_advisory_services' => $this->number_of_advisory_services,
            'number_of_services' => $this->number_of_services,
            'number_of_reservations' => $this->number_of_reservations,
            'subscribed' => Auth::guard("api_account")->check() ? $this->isUserSubscribed(Auth::guard("api_account")->user()) : false,
            'services' => $this->mapWithBookings(ServicesResource::collection($this->services)),
            'advisoryServicesTypes' => $this->mapWithBookings(AdvisoryServicesTypesResource::collection($this->advisoryServices)),
            'reservations' => $this->mapWithBookings(ReservationTypeResource::collection($this->reservations)),
