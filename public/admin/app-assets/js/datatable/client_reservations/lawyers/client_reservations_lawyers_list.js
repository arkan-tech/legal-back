...
$(function () {
    ...
    var table1 = $('#client_reservations_lawyers_list_table').DataTable(
        {
            processing: true,
            serverSide: true,
            colReorder: true,
    ...
    $(document).on('click', '.btn-delete-lawyer-reservation', function (e) {
        e.preventDefault();
        ...
        $('.confirm_btn_client_reservations').on('click', function (t) {
            ...
            $.ajax({
                ...
                type: 'get',
                success: function (response) {
                    $('body #btn_delete_lawyer_reservation_' + pid).closest('tr').css('background', '#714f4f').delay(500).hide(500);
                    setTimeout(() => {
                        Swal.fire({
                            icon: "success",
                            title: "تم الحذف !",
