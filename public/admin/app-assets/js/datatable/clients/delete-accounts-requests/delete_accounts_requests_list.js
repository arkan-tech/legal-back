...
$(function () {
    ...
    var table1 = $('#delete_account_request_list_table').DataTable(
        {
            processing: true,
            serverSide: true,
            colReorder: true,
            ...
            ajax: 'delete-accounts-requests',
            columns: cols
        });


    $(document).on('click', '.btn-delete-client-delete-account-request', function (e) {
        e.preventDefault();
        ...
        $('.confirm_btn_delete_client_delete_account_request').on('click', function (t) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    $('body #btn_delete_client_delete_account_request_' + pid).closest('tr').css('background', '#714f4f').delay(500).hide(500);
                    setTimeout(() => {
                        Swal.fire({
                            icon: "success",
                            title: "تم الحذف !",
    ...
    $(document).on('click', '.btn-show-client-delete-account-request', function (e) {
        e.preventDefault();
        let btn = $(this);
        ...
        $.ajax({
            ...
            type: 'get',
            success: function (response) {
                $('#show_client_delete_account_request_modal #client_name').html(response.item.client.myname);
                $('#show_client_delete_account_request_modal #client_phone').html(response.item.client.mobil);
                $('#show_client_delete_account_request_modal #client_email').html(response.item.client.email);
