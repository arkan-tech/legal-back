...
$(function () {
    var cols = [
        ...
        {data: 'orgnaizer', name: 'orgnaizer'},
        {data: 'location', name: 'location'},
        {data: 'price', name: 'price'},
        {data: 'action', name: 'action', orderable: false, searchable: false},
    ]

    var table1 = $('#training_list_table').DataTable(
        {
            processing: true,
            serverSide: true,
            colReorder: true,
    ...
    $(document).on('click', '.btn-delete-training', function (e) {
        ...
        Swal.fire({
            ...
            customClass: {
                confirmButton: "btn btn-primary confirm_btn_delete_lib_training",
                cancelButton: "btn btn-outline-danger ms-1"
            },
            buttonsStyling: !1
        });
        $('.confirm_btn_delete_lib_training').on('click', function (t) {
            t.preventDefault();
            $.ajax({
                url: actionUrl,
                type: 'get',
                success: function (response) {
                    $('body #btn_delete_training_' + pid).closest('tr').css('background', '#714f4f').delay(500).hide(500);
                    setTimeout(() => {
                        Swal.fire({
                            icon: "success",
                            title: "تم الحذف !",
